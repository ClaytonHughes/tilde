...
# except ImportError:
#     import subprocess
#     subprocess.run([sys.executable, '-m', 'pip', 'install', '--user', 'command_runner'])
#     from command_runner import command_runner
...
# you might need to also close and replace the old ones? idk, this is some cargo culting, sorta
UNBUFFERED = 0
unbuffered_stdout = os.fdopen(sys.stdout.fileno(), 'wb', UNBUFFERED)
unbuffered_stderr = os.fdopen(sys.stderr.fileno(), 'wb', UNBUFFERED)

# import ctypes
# _argv = ctypes.POINTER(ctypes.c_wchar_p)()
# _argc = ctypes.c_int()
# ctypes.pythonapi.Py_GetArgcArgv(ctypes.byref(_argc), ctypes.byref(_argv))
# for i in range(_argc.value):
#     print(_argv[i])
# exit()

# This could be a named tuple, but then I thought about supporting mutability in the case where
# we want to concat a second line that got wrapped somehow (because I used to occasionally see those...)
@dataclass
class Line:
    formatted: str
    _timestamp: timedelta
    @property
    def plaintext(self):
        return _remove_ansi_controls(self.formatted)
    @property
    def timestamp(self):
        return self._timestamp
    def __str__(self):
        # confusingly, the 6 here represents the *total* length, so we're requesting at least 2 digits on left: (6 - 1 decimal - 3 digits on right). IDK why it's not {:02.3f}.
        return f'{self.timestamp.total_seconds():06.3f} | {self.formatted}'
...
_nouns = {
    'lines': Noun('line', 'lines'),
    'resources': Noun('resource', 'resources'),
    'data': Noun('data', 'data'),
    'modules': Noun('module', 'modules'),
    'providers': Noun('provider', 'providers'),
}


def _escape_dquotes(value):
    return re.sub(r'"', '\\"', value)

def _dquote(value):
    return f'"${_escape_dquotes(value)}"'

def _dquote_if_needed(value):
    if any(must_escape in value for must_escape in '\' "'):
        return _dquote(value)
    return value
...
def main():
    ...
    diff_noise_file = None
    first_stderr_line = None
    errors = []
    lock = Lock()
    total_lines_written = 0
    lines_saved = 0
    verbose_lines_saved = 0
    linetypes_seen = []

    def stderr_cb(formatted_line):
        nonlocal total_lines_written
        nonlocal first_stderr_line
        with lock:
            line = Line(formatted_line, datetime.utcnow() - start)
            sys.stderr.write(str(line))
            errors.append(line)
            if first_stderr_line is None and line.plaintext not in [None, '', '\n']:
                first_stderr_line = total_lines_written
            total_lines_written += 1

    def stdout_cb(formatted_line):
        nonlocal total_lines_written
        nonlocal lines_saved
        nonlocal verbose_lines_saved
        with lock:
            # we could check for things that look like errors here, too, in case they show up on stdout and not just stderr...
            line = Line(formatted_line, datetime.utcnow() - start)
            line_type = classify_line(line.plaintext)
            if line_type == 'interesting':
                sys.stdout.write(str(line))
                total_lines_written += 1
            elif line_type not in linetypes_seen:
                if line_type != 'Still Reading...': # This could be useful to keep logging...
                    linetypes_seen.append(line_type)
                line.formatted = line_type
    ...
    if 0 < lines_saved or 0 < verbose_lines_saved:
        ...
        if verbose_lines_saved and diff_noise_file:
            print(f'Check log file {diff_noise_file} to see the omitted lines that were categorized as irrelevant diff noise')
            total_lines_written += 1
        elif verbose_lines_saved:
            print(f'You can run again with --log[=.tf-verbose] to log the omitted lines that were categorized as irrelevant diff noise')
            total_lines_written += 1


    if first_stderr_line: # don't count empty lines when checking if there was stderr output
        # if all of stderr is still on the screen, don't repeat it.
        lines_since_first_stderr = total_lines_written - first_stderr_line
        if os.get_terminal_size().lines < lines_since_first_stderr:
            print()
            print(f'{_ansi_red}! stderr was:{_ansi_reset}')
            for e in errors:
                sys.stdout.write(str(e))

        recommendations = [r for r in [_get_err_recommendation(e.plaintext) for e in errors] if r]
        if 0 < len(recommendations):
            print()
            print(f'{_ansi_orange}Recommended Actions:{_ansi_reset}')
            for recco in recommendations:
                print(f'  {_ansi_cyan}*{_ansi_reset} {recco}')
    ...
    #     if is_wrapped_line(line.text):
    #         process_line(line, is_wrapped=True)
    #         continue
...
def is_wrapped_line(line):
    # TODO: ???? How to detect maybe? Does this even happen anymore?
    return False

_resource_types = [Noun('module', 'modules'), Noun('data', 'data'), Noun('provider', 'providers')] # I don't think 'resource_types' is quite the right word...

def first_or_default(iterable, predicate, default=None):
    return next((x for x in iterable if predicate(x)), default)

def classify_line(line):
    provider_type = first_or_default([_nouns['data'], _nouns['modules']], lambda hint: line.startswith(hint.singular))
    if provider_type:
        return (
            f'Refreshing {provider_type.singular} state... ' if 'Refreshing state...' in line else
            f'Reading {provider_type.plural}...' if 'Reading...' in line or 'Read complete after' in line else
            f'Still Reading...' if 'Still reading...' in line else
            'interesting'
        )
    return 'interesting'
...
def suggest_fixes():
    ...
    for formatted_line in sys.stdin:
        ...
        print(f'{_ansi_cyan}^^^ err input line {line_no}: "{_ansi_reset}{line}{_ansi_cyan}" $$${_ansi_reset}')
...
_ansi_grey = _make_ansi(1,30)
_ansi_red = _make_ansi(31)
...
########
# this is command_runner except it's going to prefer reading stderr to stdout, to try to keep those chunks together:
